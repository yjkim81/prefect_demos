...
def data_pipeline(run: int, fail_at_run: int | None = None) -> None:
    """Main flow for data processing"""
    process_data(run, fail_at_run)


async def create_runs(
    deployment_id: str, num_runs: int, fail_at_run: int | None
) -> None:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--name",
        type=str,
        default="pipeline-deployment",
        help="Deployment name (default: pipeline-deployment)",
    )
    parser.add_argument(
        "--runs",
        type=int,
        default=10,
        help="Number of flow runs to generate (default: 10)",
    )
    parser.add_argument(
        "--fail-at-run", type=int, help="Run number at which to start failing"
    )
    parser.add_argument(
        "--tags",
        type=str,
        default="team-a,red",
        help="Comma-separated list of tags (default: team-a,red)",
    )

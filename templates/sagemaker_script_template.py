...
if __name__ == "__main__":
    ...
    train_data = pd.read_csv(
        os.path.join(args.train, "train.csv"), names=column_names, header=None
    )
    validation_data = pd.read_csv(
        os.path.join(args.validation, "test.csv"), names=column_names, header=None
    )
    ...
    s3_client.upload_file(model_location, bucket_name, filename)

...
def analyze_repo_health(repos: List[str]) -> None:
...
@flow
def fetch_repo_issues(repo: str) -> List[Dict[str, Any]]:
    ...
    for issue in all_issues[:5]:  # Limit to 5 issues to avoid hitting rate limits
        issue_details.append(fetch_issue_details.submit(repo, issue["number"]))
...
def fetch_page_of_issues(repo: str, page: int = 1) -> Optional[List[Dict[str, Any]]]:
...
def fetch_issue_details(repo: str, issue_number: int) -> Dict[str, Any]:
    ...
    return cast(Dict[str, Any], issue_data)
...
def fetch_comments(comments_url: str) -> List[GitHubComment]:
    """Fetch comments for an issue"""
    rate_limit("github-api")
    try:
        response = httpx.get(comments_url)
        response.raise_for_status()
        return cast(List[GitHubComment], response.json())
    except Exception as e:
...
@task
def calculate_response_times(issues: List[Dict[str, Any]]) -> float:
...
@task
def calculate_resolution_rate(issues: List[Dict[str, Any]]) -> float:
...
if __name__ == "__main__":
    analyze_repo_health(
        ["PrefectHQ/prefect", "pydantic/pydantic", "huggingface/transformers"]
    )
